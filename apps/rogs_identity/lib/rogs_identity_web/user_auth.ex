defmodule RogsIdentityWeb.UserAuth do
  ...
  def disconnect_sessions(tokens) do
    Enum.each(tokens, fn %{token: token} ->
      RogsIdentityWeb.Endpoint.broadcast(user_session_topic(token), "disconnect", %{})
    end)
  end
  ...
  the `current_scope`:
      ...
      defmodule RogsIdentityWeb.PageLive do
        use RogsIdentityWeb, :live_view

        on_mount {RogsIdentityWeb.UserAuth, :mount_current_scope}
        ...
      end

  Or use the `live_session` of your router to invoke the on_mount callback:

      live_session :authenticated, on_mount: [{RogsIdentityWeb.UserAuth, :require_authenticated}] do
